    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (loginError) {
                ...
                router.refresh() // Refresh to update server-side components and layout state
            }
        } catch (err) {
        ...
        <div className="min-h-screen bg-background flex flex-col justify-center py-12 sm:px-6 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                ...
                <p className="mt-2 text-center text-sm text-muted-foreground">
                    Or{" "}
                    <Link href={`/signup${redirectTo !== "/" ? `?redirect=${encodeURIComponent(redirectTo)}` : ""}`} className="font-medium text-primary hover:text-primary/80 transition-colors">
